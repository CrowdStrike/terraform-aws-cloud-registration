  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        Effect = "Allow"
        Resource = [
          "arn:${local.aws_partition}:logs:*:*:log-group:/aws/lambda/${var.resource_prefix}lambda-*",
          "arn:${local.aws_partition}:logs:*:*:log-group:/aws/lambda/${var.resource_prefix}lambda-*:log-stream:*"
        ]
        Action = [
          "logs:PutLogEvents",
